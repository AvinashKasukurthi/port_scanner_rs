        ...
        if let Ok(ip_addr) = IpAddr::from_str(&f) {
            ...
            if flag.contains("-h") || flag.contains("-help") && args.len() == 2 {
                ...
                let ip_addr = match IpAddr::from_str(&args[3]) {
                    Ok(s) => s,
                    Err(_) => return Err("Not a valid IP address; must IPv4 or IPv6"),
                };
...
fn scan(tx: Sender<u16>, start_post: u16, addr: IpAddr, num_threads: u16) {
    let mut port: u16 = start_post + 1;
